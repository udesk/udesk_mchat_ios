...
@implementation Udesk_YYKeyboardManager {
...
+ (instancetype)defaultManager {
    static Udesk_YYKeyboardManager *mgr;
...
- (void)addObserver:(id<Udesk_YYKeyboardObserver>)observer {
    if (!observer) return;
    [_observers addObject:observer];
}

- (void)removeObserver:(id<Udesk_YYKeyboardObserver>)observer {
    if (!observer) return;
...
- (void)_notifyAllObservers {
    ...
    UdeskYYKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<Udesk_YYKeyboardObserver> observer in _observers.copy) {
