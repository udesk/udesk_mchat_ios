...
static UIApplication *_UMCYYSharedApplication() {
...
@interface UMC_YYWebImageApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end
@implementation UMC_YYWebImageApplicationNetworkIndicatorInfo
@end

@implementation UMC_YYWebImageManager

+ (instancetype)sharedManager {
    static UMC_YYWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        UMC_YYImageCache *cache = [UMC_YYImageCache sharedCache];
...
- (instancetype)initWithCache:(UMC_YYImageCache *)cache queue:(NSOperationQueue *)queue{
    self = [super init];
    if (!self) return nil;
    _cache = cache;
    _queue = queue;
    _timeout = 15.0;
    if (UMC_YYImageWebPAvailable()) {
...
- (UMC_YYWebImageOperation *)requestImageWithURL:(NSURL *)url
    ...
    request.HTTPShouldHandleCookies = (options & UMC_YYWebImageOptionHandleCookies) != 0;
    request.allHTTPHeaderFields = [self headersForURL:url];
    request.HTTPShouldUsePipelining = YES;
    request.cachePolicy = (options & UMC_YYWebImageOptionUseNSURLCache) ?
        NSURLRequestUseProtocolCachePolicy : NSURLRequestReloadIgnoringLocalCacheData;
    
    UMC_YYWebImageOperation *operation = [[UMC_YYWebImageOperation alloc] initWithRequest:request
                                                                          options:options
                                                                            cache:_cache
...
+ (UMC_YYWebImageApplicationNetworkIndicatorInfo *)_networkIndicatorInfo {
    return objc_getAssociatedObject(self, @selector(_networkIndicatorInfo));
}

+ (void)_setNetworkIndicatorInfo:(UMC_YYWebImageApplicationNetworkIndicatorInfo *)info {
    objc_setAssociatedObject(self, @selector(_networkIndicatorInfo), info, OBJC_ASSOCIATION_RETAIN);
}

+ (void)_delaySetActivity:(NSTimer *)timer {
    UIApplication *app = _UMCYYSharedApplication();
    if (!app) return;
...
+ (void)_changeNetworkActivityCount:(NSInteger)delta {
    if (!_UMCYYSharedApplication()) return;
    
    void (^block)(void) = ^{
        UMC_YYWebImageApplicationNetworkIndicatorInfo *info = [self _networkIndicatorInfo];
        if (!info) {
            info = [UMC_YYWebImageApplicationNetworkIndicatorInfo new];
