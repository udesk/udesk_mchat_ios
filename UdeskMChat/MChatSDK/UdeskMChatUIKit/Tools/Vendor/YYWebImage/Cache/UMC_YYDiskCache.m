...
static UMC_YYDiskCache *_UdeskYYDiskCacheGetGlobal(NSString *path) {
...
static void _UdeskYYDiskCacheSetGlobal(UMC_YYDiskCache *cache) {
...
@implementation UMC_YYDiskCache {
    UMC_YYKVStorage *_kv;
...
- (instancetype)initWithPath:(NSString *)path
    ...
    UMC_YYDiskCache *globalCache = _UdeskYYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    UMC_YYKVStorageType type;
    if (threshold == 0) {
        type = UMC_YYKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = UMC_YYKVStorageTypeSQLite;
    } else {
        type = UMC_YYKVStorageTypeMixed;
    }
    
    UMC_YYKVStorage *kv = [[UMC_YYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    UMC_YYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [UMC_YYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [UMC_YYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != UMC_YYKVStorageTypeSQLite) {
...
- (NSString *)filePathForkey:(NSString *)key
{
    if (!key) return nil;
    Lock();
    UMC_YYKVStorageItem *item = [_kv getItemInfoForKey:key];
    Unlock();
