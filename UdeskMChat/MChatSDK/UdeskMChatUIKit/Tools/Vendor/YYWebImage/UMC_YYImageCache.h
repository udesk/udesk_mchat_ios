...
@class UMC_YYMemoryCache, UMC_YYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, UMC_YYImageCacheType) {
    /// No value.
    UMC_YYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    UMC_YYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    UMC_YYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    UMC_YYImageCacheTypeAll    = UMC_YYImageCacheTypeMemory | UMC_YYImageCacheTypeDisk,
};
 ...
 */
@interface UMC_YYImageCache : NSObject
...
@property (strong, readonly) UMC_YYMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) UMC_YYDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(UMC_YYImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(UMC_YYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(UMC_YYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, UMC_YYImageCacheType type))block;
