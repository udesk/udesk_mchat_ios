...
#import "UMC_YYAnimatedImageView.h"
 ...
 */
@interface UMC_YYImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation UMC_YYImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[UMC_YYImageWeakProxy alloc] initWithTarget:target];
}
...
@interface UMC_YYAnimatedImageView() {
    @package
...
@interface UMC_YYAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) UMC_YYAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <UdeskYYAnimatedImage> *curImage;
@end

@implementation UMC_YYAnimatedImageViewFetchOperation
- (void)main {
    __strong UMC_YYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation UMC_YYAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[UMC_YYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)step:(CADisplayLink *)link {
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        UMC_YYAnimatedImageViewFetchOperation *operation = [UMC_YYAnimatedImageViewFetchOperation new];
        operation.view = self;
