...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *udesk_umc_yy_png_copy_frame_data_at_index(const uint8_t *data,
...
CGColorSpaceRef UMC_YYCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef UMC_YYCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL UMC_YYCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, UMC_YYCGColorSpaceGetDeviceRGB());
}

BOOL UMC_YYCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, UMC_YYCGColorSpaceGetDeviceGray());
}
...
static BOOL UdeskYYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = UMC_YYCGColorSpaceGetDeviceRGB();
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef UMC_YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef UMC_YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, UMC_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, UMC_YYCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation UMC_YYUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger UMC_YYUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef UMC_YYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    return UMC_YYCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

UMC_YYImageType UMC_YYImageDetectType(CFDataRef data) {
    if (!data) return UMC_YYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return UMC_YYImageTypeUnknown;
    ...
    switch (magic4) {
        ...
        case YY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return UMC_YYImageTypeICNS;
        } break;
            
        case YY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return UMC_YYImageTypeGIF;
        } break;
            
        case YY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == YY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return UMC_YYImageTypePNG;
            }
        } break;
    ...
    switch (magic2) {
        ...
        case YY_TWO_CC('C', 'P'): { // BMP
            return UMC_YYImageTypeBMP;
        }
        case YY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return UMC_YYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return UMC_YYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return UMC_YYImageTypeJPEG2000;
    
    return UMC_YYImageTypeUnknown;
}

CFStringRef UMC_YYImageTypeToUTType(UMC_YYImageType type) {
    switch (type) {
        case UMC_YYImageTypeJPEG: return kUTTypeJPEG;
        case UMC_YYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case UMC_YYImageTypeTIFF: return kUTTypeTIFF;
        case UMC_YYImageTypeBMP: return kUTTypeBMP;
        case UMC_YYImageTypeICO: return kUTTypeICO;
        case UMC_YYImageTypeICNS: return kUTTypeAppleICNS;
        case UMC_YYImageTypeGIF: return kUTTypeGIF;
        case UMC_YYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

UMC_YYImageType UMC_YYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(UMC_YYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(UMC_YYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(UMC_YYImageTypeTIFF),
                (id)kUTTypeBMP : @(UMC_YYImageTypeBMP),
                (id)kUTTypeICO : @(UMC_YYImageTypeICO),
                (id)kUTTypeAppleICNS : @(UMC_YYImageTypeICNS),
                (id)kUTTypeGIF : @(UMC_YYImageTypeGIF),
                (id)kUTTypePNG : @(UMC_YYImageTypePNG)};
    });
    if (!uti) return UMC_YYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *UMC_YYImageTypeGetExtension(UMC_YYImageType type) {
    switch (type) {
        case UMC_YYImageTypeJPEG: return @"jpg";
        case UMC_YYImageTypeJPEG2000: return @"jp2";
        case UMC_YYImageTypeTIFF: return @"tiff";
        case UMC_YYImageTypeBMP: return @"bmp";
        case UMC_YYImageTypeICO: return @"ico";
        case UMC_YYImageTypeICNS: return @"icns";
        case UMC_YYImageTypeGIF: return @"gif";
        case UMC_YYImageTypePNG: return @"png";
        case UMC_YYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef UMC_YYCGImageCreateEncodedData(CGImageRef imageRef, UMC_YYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == UMC_YYImageTypeWebP) {
    ...
    CFStringRef uti = UMC_YYImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL Udesk_YYImageWebPAvailable() {
    return YES;
}
...
BOOL UMC_YYImageWebPAvailable() {
    return NO;
}

CFDataRef UMC_YYCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, UMC_YYImagePreset preset) {
    NSLog(@"WebP decoder is disabled");
    return NULL;
}

NSUInteger UMC_YYImageGetWebPFrameCount(CFDataRef webpData) {
    NSLog(@"WebP decoder is disabled");
    return 0;
}

CGImageRef UMC_YYCGImageCreateWithWebPData(CFDataRef webpData,
                                             BOOL decodeForDisplay,
                                             BOOL useThreads,
                                             BOOL bypassFiltering,
                                             BOOL noFancyUpsampling) {
...
@implementation UMC_YYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    UMC_YYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    UMC_YYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface UMC_YYImageDecoderFrame : UMC_YYImageFrame
...
@implementation UMC_YYImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    UMC_YYImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation UMC_YYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    UMC_YYImageDecoder *decoder = [[UMC_YYImageDecoder alloc] initWithScale:scale];
...
- (UMC_YYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    UMC_YYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((UMC_YYImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    UMC_YYImageType type = UMC_YYImageDetectType((__bridge CFDataRef)data);
...
- (UMC_YYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    UMC_YYImageDecoderFrame *frame = [(UMC_YYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != UMC_YYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = UMC_YYCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == UMC_YYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = UMC_YYImageDisposeNone;
        frame.blend = UMC_YYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case UMC_YYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case UMC_YYImageTypePNG: {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            ...
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = UMC_YYImageBlendOver;
            }
...
- (void)_updateSourceAPNG {
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        UMC_YYImageDecoderFrame *frame = [UMC_YYImageDecoderFrame new];
        [frames addObject:frame];
        ...
        switch (fi->frame_control.dispose_op) {
            case UDESK_YY_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = UMC_YYImageDisposeBackground;
            } break;
            case UDESK_YY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = UMC_YYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = UMC_YYImageDisposeNone;
            } break;
        ...
        if (frame.blend == UMC_YYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != UMC_YYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == UMC_YYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == UMC_YYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == UMC_YYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = UMC_YYUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    UMC_YYImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = UMC_YYCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = udesk_umc_yy_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(UMC_YYImageDecoderFrame *)frame {
    if (frame.dispose == UMC_YYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == UMC_YYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == UMC_YYImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(UMC_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == UMC_YYImageDisposePrevious) {
        if (frame.blend == UMC_YYImageBlendOver) {
...
@implementation UMC_YYImageEncoder {
...
- (instancetype)init {
    ...
    return [self initWithType:UMC_YYImageTypeUnknown];
}

- (instancetype)initWithType:(UMC_YYImageType)type {
    if (type == UMC_YYImageTypeUnknown || type >= UMC_YYImageTypeOther) {
    ...
    switch (type) {
        case UMC_YYImageTypeJPEG:
        case UMC_YYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case UMC_YYImageTypeTIFF:
        case UMC_YYImageTypeBMP:
        case UMC_YYImageTypeGIF:
        case UMC_YYImageTypeICO:
        case UMC_YYImageTypeICNS:
        case UMC_YYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, UMC_YYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, UMC_YYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == UMC_YYImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = UMC_YYCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return UMC_YYCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return UMC_YYCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == UMC_YYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = UMC_YYCGImageCreateEncodedData(decoded, UMC_YYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, UMC_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = UMC_YYCGImageCreateEncodedData(extendedImage, UMC_YYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)encode {
    ...
    if (_type == UMC_YYImageTypePNG) return [self _encodeAPNG];
    if (_type == UMC_YYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(UMC_YYImageType)type quality:(CGFloat)quality {
    UMC_YYImageEncoder *encoder = [[UMC_YYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(UMC_YYImageDecoder *)decoder type:(UMC_YYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (UMC_YYImageCoder)

- (instancetype)yy_imageByDecoded {
    if (self.yy_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = UMC_YYCGImageCreateDecodedCopy(imageRef, YES);
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[UMC_YYSpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[UMC_YYImage class]]) {
        UMC_YYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == UMC_YYImageTypeGIF ||
                    image.animatedImageType == UMC_YYImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = UMC_YYCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
