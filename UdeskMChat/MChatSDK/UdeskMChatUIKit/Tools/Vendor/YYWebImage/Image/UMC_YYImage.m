...
@implementation UMC_YYImage {
    UMC_YYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (UMC_YYImage *)imageNamed:(NSString *)name {
...
+ (UMC_YYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (UMC_YYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (UMC_YYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        UMC_YYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
