...
static BOOL UdeskYYCGImageLastPixelFilled(CGImageRef image) {
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, UMC_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface UMC_YYWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation UMC_YYWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[UMC_YYWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface UMC_YYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) UMC_YYImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) UMC_YYWebImageProgressBlock progress;
@property (nonatomic, copy) UMC_YYWebImageTransformBlock transform;
@property (nonatomic, copy) UMC_YYWebImageCompletionBlock completion;
@end


@implementation UMC_YYWebImageOperation
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & UMC_YYWebImageOptionShowNetworkActivity)) {
                [UMC_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, UMC_YYWebImageFromNone, UMC_YYWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & UMC_YYWebImageOptionUseNSURLCache) &&
            !(_options & UMC_YYWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:UMC_YYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, UMC_YYWebImageFromMemoryCache, UMC_YYWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & UMC_YYWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:UMC_YYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:UMC_YYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & UMC_YYWebImageOptionIgnoreFailedURL) && UdeskURLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, UMC_YYWebImageFromNone, UMC_YYWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[UMC_YYWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & UMC_YYWebImageOptionShowNetworkActivity)) {
                [UMC_YYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, UMC_YYWebImageFromNone, UMC_YYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, UMC_YYWebImageFromDiskCache, UMC_YYWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & UMC_YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([UMC_YYWebImageOperation _imageQueue], ^{
                        [_cache setImage:image imageData:data forKey:_cacheKey withType:UMC_YYImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            ...
            if (!image) {
                ...
                if (_options & UMC_YYWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, UMC_YYWebImageFromRemote, UMC_YYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & UMC_YYWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & UMC_YYWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & UMC_YYWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & UMC_YYWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[UMC_YYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == UMC_YYImageTypeUnknown ||
            _progressiveDecoder.type == UMC_YYImageTypeWebP ||
            _progressiveDecoder.type == UMC_YYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != UMC_YYImageTypeJPEG &&
                _progressiveDecoder.type != UMC_YYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            UMC_YYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, UMC_YYWebImageFromRemote, UMC_YYWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == UMC_YYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, UMC_YYWebImageFromRemote, UMC_YYWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & UMC_YYWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & UMC_YYWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[UMC_YYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((UMC_YYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    UMC_YYImageDecoder *decoder = [UMC_YYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                UMC_YYImageType imageType = UMC_YYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case UMC_YYImageTypeJPEG:
                    case UMC_YYImageTypeGIF:
                    case UMC_YYImageTypePNG:
                    case UMC_YYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == UMC_YYImageTypeGIF ||
                                imageType == UMC_YYImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & UMC_YYWebImageOptionShowNetworkActivity)) {
                [UMC_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, UMC_YYWebImageFromNone, UMC_YYWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & UMC_YYWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & UMC_YYWebImageOptionAllowBackgroundTask) && _UdeskYYSharedApplication()) {
