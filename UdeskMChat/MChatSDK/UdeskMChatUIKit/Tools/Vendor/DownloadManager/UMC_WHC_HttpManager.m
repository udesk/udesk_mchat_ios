...
//默认下载并发数量 在这里修改
const NSInteger kUMCWHCDefaultDownloadNumber = 5;

@interface UMC_WHC_HttpManager () {
...
@implementation UMC_WHC_HttpManager

+ (nonnull instancetype)shared {
    static  UMC_WHC_HttpManager * WHCHttpManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        WHCHttpManager = [UMC_WHC_HttpManager new];
    });
...
- (nullable UMC_WHC_HttpOperation *)get:(nonnull NSString *)strUrl
               didFinished:(nullable UMC_WHCDidFinished)finishedBlock {
    return [self get:strUrl process:nil didFinished:finishedBlock];
}

- (nullable UMC_WHC_HttpOperation *)get:(nonnull NSString *)strUrl
                   process:(nullable UMC_WHCProgress) processBlock
               didFinished:(nullable UMC_WHCDidFinished)finishedBlock {
    UMC_WHC_HttpOperation * getOperation = nil;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        getOperation = [UMC_WHC_HttpOperation new];
        getOperation.requestType = UMC_WHCHttpRequestGet;
        getOperation.progressBlock = processBlock;
        getOperation.strUrl = strUrl;
        __weak typeof(self) weakSelf = self;
        getOperation.didFinishedBlock = ^(UMC_WHC_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
...
- (nullable UMC_WHC_HttpOperation *)post:(nonnull NSString *)strUrl
...
- (nullable UMC_WHC_HttpOperation *)post:(nonnull NSString *)strUrl
                    ...
                    process:(nullable UMC_WHCProgress) processBlock
                didFinished:(nullable UMC_WHCDidFinished)finishedBlock {
    UMC_WHC_HttpOperation * postOperation = nil ;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        postOperation = [UMC_WHC_HttpOperation new];
        postOperation.requestType = UMC_WHCHttpRequestPost;
        postOperation.progressBlock = processBlock;
        postOperation.postParam = param;
        postOperation.strUrl = strUrl;
        __weak typeof(self) weakSelf = self;
        postOperation.didFinishedBlock = ^(UMC_WHC_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
            if (!isSuccess && error.code == 404) {
                [weakSelf.failedUrls addObject:strUrl];
            }
            if (finishedBlock) {
...
- (nullable UMC_WHC_HttpOperation *)upload:(nonnull NSString *)strUrl
...
- (nullable UMC_WHC_HttpOperation *)upload:(nonnull NSString *)strUrl
    ...
    UMC_WHC_HttpOperation * uploadOperation = nil ;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        uploadOperation = [UMC_WHC_HttpOperation new];
        [self setHttpOperation:uploadOperation];
        uploadOperation.requestType = UMC_WHCHttpRequestFileUpload;
        uploadOperation.progressBlock = processBlock;
        uploadOperation.strUrl = strUrl;
        uploadOperation.postParam = _uploadPostData;
        __weak typeof(self) weakSelf = self;
        uploadOperation.didFinishedBlock = ^(UMC_WHC_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
        ...
        uploadOperation.contentType = [NSString stringWithFormat:@"multipart/form-data; charset=%@; boundary=%@", charset, kUMCWHCUploadCode];
...
- (nullable UMC_WHC_DownloadOperation *)download:(nonnull NSString *)strUrl
                                    savePath:(nonnull NSString *)savePath
                                    delegate:(nullable id<UMC_WHC_DownloadDelegate>)delegate {
...
- (nullable UMC_WHC_DownloadOperation *)download:(nonnull NSString *)strUrl
    ...
    UMC_WHC_DownloadOperation  * downloadOperation = nil;
    ...
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        ...
        for (UMC_WHC_DownloadOperation * tempDownloadOperation in _fileDownloadOperationQueue.operations) {
        ...
        if([self createFileSavePath:savePath]) {
            downloadOperation = [UMC_WHC_DownloadOperation new];
            downloadOperation.requestType = UMC_WHCHttpRequestGet;
...
- (nullable UMC_WHC_DownloadOperation *)download:(nonnull NSString *)strUrl
                           savePath:(nonnull NSString *)savePath
                            response:(nullable UMC_WHCResponse) responseBlock
                            process:(nullable UMC_WHCProgress) processBlock
                        didFinished:(nullable UMC_WHCDidFinished) finishedBlock {
...
- (nullable UMC_WHC_DownloadOperation *)download:(nonnull NSString *)strUrl
    ...
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        ...
        if([self createFileSavePath:savePath]) {
            ...
            downloadOperation.strUrl = strUrl;
            __weak typeof(self) weakSelf = self;
            downloadOperation.didFinishedBlock = ^(UMC_WHC_BaseOperation *operation,
                                                   NSData *data,
                                                   NSError *error,
                                                   BOOL isSuccess) {
                if (!isSuccess && error.code == 404) {
...
- (BOOL)waitingDownload {
    return _fileDownloadOperationQueue.operations.count > kUMCWHCDefaultDownloadNumber;
}
...
- (nullable NSString *)handleFileName:(NSString *)saveFileName url:(NSString *)strUrl {
    if (!_fileDownloadOperationQueue) {
        ...
        _fileDownloadOperationQueue.maxConcurrentOperationCount = kUMCWHCDefaultDownloadNumber;
    }
...
//返回指定文件名下载对象
- (nullable UMC_WHC_DownloadOperation *)downloadOperationWithFileName:(nonnull NSString *)fileName {
    UMC_WHC_DownloadOperation * downloadOperation = nil;
    for (UMC_WHC_DownloadOperation * tempDownloadOperation in _fileDownloadOperationQueue.operations) {
...
- (void)cancelAllDownloadTaskAndDelFile:(BOOL)isDelete {
    for (UMC_WHC_DownloadOperation * operation in _fileDownloadOperationQueue.operations) {
...
- (UMC_WHC_DownloadOperation *)replaceCurrentDownloadOperationBlockResponse:(nullable UMC_WHCResponse)responseBlock
                                             process:(nullable UMC_WHCProgress)processBlock
                                         didFinished:(nullable UMC_WHCDidFinished)didFinishedBlock
                                            fileName:(nonnull NSString *)fileName {
    for (UMC_WHC_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
...
- (UMC_WHC_DownloadOperation *)replaceCurrentDownloadOperationDelegate:(nullable id<UMC_WHC_DownloadDelegate>)delegate
                                       fileName:(nonnull NSString *)fileName {
    for (UMC_WHC_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
        if([downloadOperation.saveFileName isEqualToString:fileName]){
...
//替换所有当前下载代理
- (UMC_WHC_DownloadOperation *)replaceAllDownloadOperationBlockResponse:(nullable UMC_WHCResponse)responseBlock
                                         process:(nullable UMC_WHCProgress)processBlock
                                     didFinished:(nullable UMC_WHCDidFinished)didFinishedBlock {
...
- (UMC_WHC_DownloadOperation *)replaceAllDownloadOperationDelegate:(nullable id<UMC_WHC_DownloadDelegate>)delegate {
    if (_fileDownloadOperationQueue.operations.count > 0) {
        for (UMC_WHC_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
            downloadOperation.progressBlock = nil;
            downloadOperation.responseBlock = nil;
...
- (void)cancelHttpRequestWithUrl:(nonnull NSString *)url {
    for (UMC_WHC_BaseOperation * operation in _httpOperationQueue.operations) {
...
- (__autoreleasing NSError *)error:(nonnull NSString *)message {
    __autoreleasing NSError  * error = [[NSError alloc]initWithDomain:kUMCWHCDomain
...
- (void)setHttpOperation:(UMC_WHC_BaseOperation *)httpOperation {
...
- (void)buildMultipartFormDataPostBody {
    ...
    NSString *stringBoundary = kUMCWHCUploadCode;
