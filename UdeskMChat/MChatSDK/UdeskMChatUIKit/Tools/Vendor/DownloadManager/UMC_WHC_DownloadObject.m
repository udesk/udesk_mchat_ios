...
@implementation UMC_WHC_DownloadObject 

- (instancetype)init {
    self = [super init];
    if (self) {
        _downloadSpeed = @"0KB/s";
        _downloadState = UMC_WHCNone;
    }
    return self;
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        ...
        _downloadState = _totalLenght != 0 ? (self.currentDownloadLenght == self.totalLenght ? UMC_WHCDownloadCompleted : UMC_WHCDownloadCanceled) : UMC_WHCNone;
    }
    return self;
}
...
+ (NSString *)cachePlistPath {
    return [NSString stringWithFormat:@"%@WHCDownloadCache.plist",[UMC_WHC_DownloadObject cachePlistDirectory]];
}
...
+ (UMC_WHC_DownloadObject *)readDiskCache:(NSString *)downloadPath {
    return [NSKeyedUnarchiver unarchiveObjectWithFile:[UMC_WHC_DownloadObject getCachedFileName:downloadPath]];
}

+ (NSArray *)readDiskAllCache {
    NSMutableArray * downloadObjectArr = [NSMutableArray array];
    NSMutableDictionary * cacheDictionary = [NSMutableDictionary dictionaryWithContentsOfFile:[UMC_WHC_DownloadObject cachePlistPath]];
    if (cacheDictionary != nil) {
        NSArray * allKeys = [cacheDictionary allKeys];
        for (NSString * path in allKeys) {
            [downloadObjectArr addObject:[UMC_WHC_DownloadObject readDiskCache:path]];
        }
    }
...
+ (NSString *)getCachedFileName:(NSString *)name {
    ...
    if (name != nil) {
        ...
        return [NSString stringWithFormat:@"%@%@",[UMC_WHC_DownloadObject cacheDirectory],cachedFileName];
    }
    return [NSString stringWithFormat:@"%@WHC",[UMC_WHC_DownloadObject cacheDirectory]];
}
...
- (void)writeDiskCache {
    if (_downloadPath != nil) {
        [self createCacheDirectory:[UMC_WHC_DownloadObject cacheDirectory]];
        [NSKeyedArchiver archiveRootObject:self
                                    toFile:[UMC_WHC_DownloadObject getCachedFileName:_fileName]];//_downloadPath
        [self createCacheDirectory:[UMC_WHC_DownloadObject cachePlistDirectory]];
        NSMutableDictionary * cacheDictionary = [NSMutableDictionary dictionaryWithContentsOfFile:[UMC_WHC_DownloadObject cachePlistPath]];
        if (cacheDictionary == nil) {
            cacheDictionary = [NSMutableDictionary dictionary];
        }
        [cacheDictionary setObject:@"WHC" forKey:_fileName];//_downloadPath
        [cacheDictionary writeToFile:[UMC_WHC_DownloadObject cachePlistPath] atomically:YES];
    }
}

- (void)removeFromDisk {
    NSFileManager * fm = [NSFileManager defaultManager];
    if ([fm fileExistsAtPath:[UMC_WHC_DownloadObject getCachedFileName:_fileName]]) {//_downloadPath
        [fm removeItemAtPath:[UMC_WHC_DownloadObject getCachedFileName:_fileName] error:nil];//_downloadPath
        NSMutableDictionary * cacheDictionary = [NSMutableDictionary dictionaryWithContentsOfFile:[UMC_WHC_DownloadObject cachePlistPath]];
        if (cacheDictionary != nil) {
            [cacheDictionary removeObjectForKey:_fileName];//_downloadPath
            [cacheDictionary writeToFile:[UMC_WHC_DownloadObject cachePlistPath] atomically:YES];
        }
        if ([fm fileExistsAtPath:[NSString stringWithFormat:@"%@%@",[UMC_WHC_DownloadObject videoDirectory],_fileName]]) {
            [fm removeItemAtPath:[NSString stringWithFormat:@"%@%@",[UMC_WHC_DownloadObject videoDirectory],_fileName] error:nil];
        }
    }
}
