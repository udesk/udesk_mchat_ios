...
@interface UMC_WHC_DownloadOperation () {
...
@implementation UMC_WHC_DownloadOperation
...
- (void)start {
    ...
    if(![fm fileExistsAtPath:self.saveFilePath]) {
        ...
        NSString  * strRange = [NSString stringWithFormat:kUMCWHCRequestRange ,_localFileSizeLenght];
    ...
    if(error == nil) {
        ...
        NSLog(@"UdeskSDK：%@",kUMCWHCCalculateFolderSpaceAvailableFailError);
    }
    [super start];
    [self startRequest];
}
...
- (void)cancelDownloadTaskAndDeleteFile:(BOOL)isDelete {
    ...
    NSError * error = nil;
    if (!isDelete) {
        error = [NSError errorWithDomain:kUMCWHCDomain
                            code:[self getCode]
...
- (void)handleReqeustError:(NSError *)error code:(NSInteger)code {
    if (code != UMC_WHCCancelDownloadError) {
        [self removeDownloadFile];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if (![self handleResponseError:response]){
        ...
        if([self calculateFreeDiskSpace] < _actualFileSizeLenght){
            error = [[NSError alloc]initWithDomain:kUMCWHCDomain
                                              code:UMC_WHCFreeDiskSpaceLack
                                          userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:kUMCWHCFreeDiskSapceError,_actualFileSizeLenght]}];
    ...
    WHC1:
        ...
        error = [NSError errorWithDomain:kUMCWHCDomain code:code userInfo:@{NSLocalizedDescriptionKey:response.description}];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if(self.responseData.length > kUMCWHCWriteSizeLenght && _fileHandle){
        [_fileHandle writeData:self.responseData];
        [self clearResponseData];
    }
    dispatch_async(dispatch_get_main_queue(), ^{
