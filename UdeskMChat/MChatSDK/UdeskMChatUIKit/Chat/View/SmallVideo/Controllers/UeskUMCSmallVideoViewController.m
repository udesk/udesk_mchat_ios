...
#import "UdeskUMCSmallVideoPreviewViewController.h"
#import "UMCSmallVideoBottomView.h"
...
@interface UeskUMCSmallVideoViewController ()<UMCSmallVideoBottomViewDelegate>

@property (nonatomic, strong) UMCSmallVideoManager *videoManager;
@property (nonatomic, strong) UMCSmallVideoBottomView *bottomView;
@property (nonatomic, strong) UMCButton *invertButton;
@property (nonatomic, strong) UMCButton *backButton;
@property (nonatomic, strong) UIView *focusView;
@property (nonatomic, strong) UIView *previewView;
@property (nonatomic, assign) BOOL   savingImage;
@property (nonatomic, strong) UdeskUMCSmallVideoPreviewViewController *previewVC;

@end

@implementation UeskUMCSmallVideoViewController
...
- (void)configVideoManager {
    ...
    _videoManager = [UMCSmallVideoManager sharedManager];
    _videoManager.maxDuration = [UMCSDKConfig sharedConfig].smallVideoDuration;
    _videoManager.cropSize = self.previewView.frame.size;
    
    __weak typeof(self) weakSelf = self;
    self.videoManager.finishBlock = ^(NSDictionary *info, UMCRecorderFinishedReason finishReason) {
...
- (void)previewVideoWithURL:(NSString *)sandboxURL videoInfo:(NSDictionary *)info {
    ...
    self.previewVC = [UdeskUMCSmallVideoPreviewViewController new];
...
#pragma mark - lazy
- (UMCButton *)invertButton {
    if (!_invertButton) {
        _invertButton = [UMCButton buttonWithType:UIButtonTypeCustom];
        [_invertButton setImage:[UIImage umcDefaultSmallVideoCameraSwitch] forState:UIControlStateNormal];
...
- (UMCButton *)backButton {
    if (!_backButton) {
        _backButton = [UMCButton buttonWithType:UIButtonTypeCustom];
        [_backButton setImage:[UIImage umcDefaultSmallVideoBack] forState:UIControlStateNormal];
...
- (UMCSmallVideoBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[UMCSmallVideoBottomView alloc] initWithFrame:CGRectMake(0,[UIScreen mainScreen].bounds.size.height - 180, [UIScreen mainScreen].bounds.size.width, 300)];
        _bottomView.backgroundColor = [UIColor clearColor];
        _bottomView.delegate = self;
        _bottomView.duration = [UMCSDKConfig sharedConfig].smallVideoDuration;
    }
    return _bottomView;
}
...
- (void)smallVideoCurrentFrame {
    ...
    AVCaptureVideoOrientation avcaptureOrientation = [UMCVideoUtil avOrientationForDeviceOrientation:curDeviceOrientation];
    ...
    [self.videoManager.imageDataOutput captureStillImageAsynchronouslyFromConnection:conntion
                                                                   completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
                                                                       ...
                                                                       [strongSelf previewPhoto:[UMCImageHelper fixOrientation:image]];
                                                                   }];
}
